...
func (f *DFA) AddInvalidWords(words []string) {
	f.l.Lock()
	defer f.l.Unlock()
	if len(words) > 0 {
		for _, s := range words {
...
func (f *DFA) Check(txt string) ([]string, bool) {
	_, found, _, exist := f.check(txt, false)
	return found, exist
}
...
func (f *DFA) check(txt string, replace bool) (dist string, found []string, target []string, exist bool) {
	var (
		str        = []rune(txt)
		ok         bool
		node       *Node
